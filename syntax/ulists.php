...
class syntax_plugin_markdownx_ulists extends DokuWiki_Syntax_Plugin {
    ...
    function connectTo($mode) {
        $this->Lexer->addEntryPattern('^[ ]{0,3}[*+-][ \t](?!(?:[ ]?[*+-][ ]?){2,}[ \t]*\n)', $mode, 'plugin_markdownx_ulists');
        $this->Lexer->addPattern('\n[ \t]*[*+-][ \t](?!(?:[ ]?[*+-][ ]?){2,}[ \t]*\n)', 'plugin_markdownx_ulists');
    }
    ...
    function handle($match, $state, $pos, Doku_Handler $handler) {
        switch ($state) {
            case DOKU_LEXER_ENTER:
                $ReWriter = new Doku_Handler_MarkdownX_Unordered_List($handler->CallWriter);
...
class Doku_Handler_MarkdownX_Unordered_List extends Doku_Handler_List {
