...
class syntax_plugin_markdownx_olists extends DokuWiki_Syntax_Plugin {
    ...
    function connectTo($mode) {
        $this->Lexer->addEntryPattern('^[ ]{0,3}\d+\.[ \t]', $mode, 'plugin_markdownx_olists');
        $this->Lexer->addPattern('\n^[ \t]*\d+\.[ \t]', 'plugin_markdownx_olists');
    }

    function postConnect() {
        $this->Lexer->addExitPattern('\n', 'plugin_markdownx_olists');
    }
    
    function handle($match, $state, $pos, Doku_Handler $handler) {
        switch ($state) {
            case DOKU_LEXER_ENTER:
                $ReWriter = new Doku_Handler_MarkdownX_Ordered_List($handler->CallWriter);
...
class Doku_Handler_MarkdownX_Ordered_List extends Doku_Handler_List {
